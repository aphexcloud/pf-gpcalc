...
export default function SettingsPage() {
  ...
  const [tempSquareSettings, setTempSquareSettings] = useState({
    accessToken: '',
    environment: 'sandbox'
  });
  const [savingSquareSettings, setSavingSquareSettings] = useState(false);
  const [squareSettingsSaved, setSquareSettingsSaved] = useState(false);
    ...
    async function loadSettings() {
      try {
        ...
        if (res.ok) {
          ...
          if (data.square) {
            setSquareSettings(data.square);
            setTempSquareSettings(data.square);
          }
          if (data.branding) {
  ...
  const saveSquareSettings = async () => {
    setSavingSquareSettings(true);
    setSquareSettingsSaved(false);
    ...
    try {
      const res = await fetch('/api/settings', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          square: tempSquareSettings
        })
      });

      if (res.ok) {
        setSquareSettings(tempSquareSettings);
        setSquareSettingsSaved(true);
        setTimeout(() => setSquareSettingsSaved(false), 3000);
      }
    } catch (err) {
      console.error('Failed to save Square settings:', err);
    } finally {
      setSavingSquareSettings(false);
    }
  };
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={tempSquareSettings.environment}
                    onChange={(e) => setTempSquareSettings({ ...tempSquareSettings, environment: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black/20 text-sm"
                  >
                    <option value="sandbox">Sandbox (Testing)</option>
                    <option value="production">Production (Live)</option>
                  </select>
                  <p className="text-xs text-gray-500 mt-1">
                    Select "Production" for live data or "Sandbox" for testing
                  </p>
                </div>

                {/* Access Token */}
                <div>
                  <label className="block text-sm text-gray-600 mb-1">Square Access Token</label>
                  <div className="relative">
                    <input
                      type={showSquareToken ? "text" : "password"}
                      value={tempSquareSettings.accessToken}
                      onChange={(e) => setTempSquareSettings({ ...tempSquareSettings, accessToken: e.target.value })}
                      placeholder="Enter your Square access token"
                      className="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black/20 text-sm font-mono"
                    />
                    <button
                      type="button"
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Get your access token from <a href="https://developer.squareup.com/apps" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">Square Developer Dashboard</a>
                  </p>
                </div>
                ...
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-xs text-yellow-800">
                  <strong>Note:</strong> Settings configured here will override Docker environment variables (SQUARE_ACCESS_TOKEN and SQUARE_ENVIRONMENT). Remove the Docker environment variables after configuring here to avoid confusion.
                </div>
              </div>
            </div>
          </div>
